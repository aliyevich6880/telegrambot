...
def add_word(word, added_by, category):
    with get_conn() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO words (chat_id, word, added_by, category) VALUES (?, ?, ?, ?)",
            (0, word, added_by, category)
        )
        return c.lastrowid
...
def list_words():
    with get_conn() as conn:
        c = conn.cursor()
        c.execute("SELECT id, category, word, added_by FROM words ORDER BY category, id")
        return c.fetchall()
...
def _words_pool(category=None):
    """Returns (pool, source_label). The word bank is global (shared across all groups):
    prefers custom words for the exact category, then any custom word, then the default bank."""
    with get_conn() as conn:
        c = conn.cursor()
        if category:
...
def pick_random_word(category, used_words):
    ...
    pool, source = _words_pool(category)
